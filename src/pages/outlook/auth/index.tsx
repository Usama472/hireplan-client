...
const OutlookAuthPage = () => {
  ...
  useEffect(() => {
    ...
    API.microsoft.connectMicrosoftCalendar(code)
      .then((response) => {
        setStatus('success');
        setMessage(`Successfully connected ${response.tokens?.email || 'Outlook Calendar'}`);
        toast({
          title: "Success!",
      ...
      .catch((error) => {
        ...
        setStatus('error');
        setMessage('Failed to connect Outlook Calendar. Please try again.');
        toast({
          title: "Connection Failed",
