              ...
              <div className="flex items-center justify-between py-4">
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex items-center gap-2 text-sm text-gray-500">
                    <span>•</span>
                    <span>{formData.positionsToHire || 1} position{(formData.positionsToHire || 1) > 1 ? 's' : ''}</span>
                    {jobViews > 0 && (
                      <>
                        <span>•</span>
                        <span>{jobViews} views</span>
                      </>
                    )}
                  </div>
                </div>
            ...
            <div className="mb-6 space-y-3">
              {issues.length > 0 && (
                <Alert variant="destructive" className="py-3">
                  ...
                  <AlertDescription className="text-sm">
                    <span className="font-medium">{issues.length} issue{issues.length > 1 ? 's' : ''} to resolve:</span>
              ...
              {warnings.length > 0 && (
                <Alert className="border-yellow-200 bg-yellow-50 py-3">
                  ...
                  <AlertDescription className="text-sm">
                    <span className="font-medium text-yellow-900">{warnings.length} recommendation{warnings.length > 1 ? 's' : ''}:</span>
                    <span className="ml-2 text-yellow-800">{warnings.join(', ')}</span>
                  </AlertDescription>
                </Alert>
              )}
            </div>
          ...
          <div className="flex gap-6">
            ...
            <div className="w-64 flex-shrink-0">
              <div className="bg-white rounded-lg border border-gray-200 shadow-sm sticky top-24">
                <div className="p-4">
                  <h3 className="text-sm font-medium text-gray-900 mb-3">Job Sections</h3>
                      ...
                      return (
                        <button
                          ...
                          className={cn(
                            ...
                            isActive
                              ? "bg-gray-50 text-gray-900 shadow-sm after:absolute after:left-0 after:right-0 after:bottom-0 after:h-0.5 after:bg-gradient-to-r after:from-blue-500 after:via-purple-500 after:to-pink-500"
