...
export function EnhancedProgressStepper({
  ...
  const handleStepClick = (stepIndex: number) => {
    if (clickable && onStepClick && (isPast(stepIndex) || isCurrent(stepIndex))) {
            ...
            {steps.map((_, index) => (
              <div
                ...
                className={cn(
                  ...
                  {
                    "border-blue-500 bg-blue-500": isCompleted(index) || isCurrent(index),
              ...
              <div className="flex flex-col items-center">
                <button
                  className={cn(
                    ...
                    {
                      'bg-blue-600 border-blue-600 text-white shadow-lg': current,
                      'bg-green-500 border-green-500 text-white': completed,
                      'bg-white border-gray-300 text-gray-500 hover:border-gray-400': future,
                      'bg-gray-100 border-gray-400 text-gray-600': past && !completed,
                ...
                >
                  {completed ? (
                    <Check className={cn('w-4 h-4', size === 'sm' && 'w-3 h-3', size === 'lg' && 'w-5 h-5')} />
                  ) : step.icon ? (
                    <step.icon className={cn('w-4 h-4', size === 'sm' && 'w-3 h-3', size === 'lg' && 'w-5 h-5')} />
                  ) : (
                    index + 1
                  )}
                </button>
            ...
            return (
              <React.Fragment key={step.id}>
                <div className="flex flex-col items-center space-y-1">
                  <button
                    className={cn(
                      'flex items-center justify-center rounded-full border-2 font-semibold transition-all duration-300 w-6 h-6 text-xs',
                      {
                        'bg-blue-600 border-blue-600 text-white shadow-lg': current,
                        'bg-green-500 border-green-500 text-white': completed,
                ...
                <button
                  className={cn(
                    ...
                    stepSizeClasses[size],
                    {
                      'bg-blue-600 border-blue-600 text-white shadow-lg': current,
                      'bg-green-500 border-green-500 text-white': completed,
                      'bg-white border-gray-300 text-gray-500 hover:border-gray-400 hover:shadow-md': future,
                      'bg-gray-100 border-gray-400 text-gray-600': past && !completed,
                      'cursor-pointer': clickable && (past || current),
                      'cursor-default': !clickable,
                    }
